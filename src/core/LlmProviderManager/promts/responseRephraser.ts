...
interface ResponseRephraserParams {
	topic: string;
	concept: string;
}

// TODO hay que re orientar la intencion de de repraser a ser la generacion de una respuesta y no solamente 
// hacer repraser de un supuesto mensaje que no existe por lo menos con la estrucutra actual 
// esto deberia generar la respuesta y formatearla de la forma correcta 
//
// hay que replicar eso para el undertanding con output en formato 
function createResponseRephraserPrompt(params: ResponseRephraserParams): string {
	let prompt = promptTemplate;
	prompt = prompt.replace("{{history}}", params.topic);
	prompt = prompt.replace("{{suggested_response}}", params.concept);

	return prompt;
}

export default createResponseRephraserPrompt;
